...
if [ ! -f "$STATE_PATH" ]; then
  echo "Claude Code installation state is missing. Run the versioned installer first: $STATE_PATH" >&2
  exit 1
fi
...
if [ "$SCHEMA_VERSION" != "1" ] || [ "$INTEGRATION" != "claude-code" ]; then
  echo "Unsupported Claude Code installation state." >&2
  exit 1
fi
...
if [ "$FORCE" != "1" ] && [ "$COMPARISON" -eq 0 ]; then
  log "Claude Code integration is already up to date at $INSTALLED_VERSION."
  exit 0
fi
if [ "$CHECK_ONLY" = "1" ]; then
  log "Claude Code integration update available: $INSTALLED_VERSION -> $TARGET_VERSION"
  exit 0
fi

ARTIFACT_NAME=$(json_claude_posix_field file "$MANIFEST_PATH")
ARTIFACT_FORMAT=$(json_claude_posix_field format "$MANIFEST_PATH")
EXPECTED_HASH=$(json_claude_posix_field sha256 "$MANIFEST_PATH" | tr 'A-F' 'a-f')
case "$ARTIFACT_NAME" in
  ""|*/*|*\\*)
    echo "POSIX Claude Code artifact name is invalid." >&2
    exit 1
    ;;
  *.tar.gz) ;;
  *)
...
if [ -z "$ARTIFACT_NAME" ] || [ "$ARTIFACT_FORMAT" != "tar.gz" ]; then
  echo "Release manifest must contain one tar.gz POSIX Claude Code artifact." >&2
  exit 1
fi
if ! printf '%s' "$EXPECTED_HASH" | grep -Eq '^[a-f0-9]{64}$'; then
  echo "POSIX Claude Code artifact checksum is invalid." >&2
  exit 1
fi
...
STAGED_CLAUDE_CODE="$STAGE_ROOT/claude-code"
for required in install.sh doctor.sh update.sh release.json; do
  [ -f "$STAGED_CLAUDE_CODE/$required" ] || {
    echo "Release artifact is missing $required." >&2
    exit 1
  }
done

CONTRACT_VERSION=$(json_field version "$STAGED_CLAUDE_CODE/release.json")
CONTRACT_TAG=$(json_field releaseTag "$STAGED_CLAUDE_CODE/release.json")
CONTRACT_INTEGRATION=$(json_field integration "$STAGED_CLAUDE_CODE/release.json")
if [ "$CONTRACT_INTEGRATION" != "claude-code" ] || [ "$CONTRACT_VERSION" != "$TARGET_VERSION" ] || [ "$CONTRACT_TAG" != "$RELEASE_TAG" ]; then
  echo "Release artifact contract does not match the manifest." >&2
  exit 1
fi

BACKUP_ROOT="$TEMP_ROOT/backup"
mkdir -p "$BACKUP_ROOT/integration"
cp -R "$INTEGRATION_HOME/." "$BACKUP_ROOT/integration/"

UPDATE_FAILED="0"
if ! ALGOMIM_HOME="$ALGOMIM_HOME" sh "$STAGED_CLAUDE_CODE/install.sh" \
  --base-url "$BASE_URL" \
  --credential-profile "$CREDENTIAL_PROFILE" \
  --release-ref "$RELEASE_TAG" \
  --release-version "$TARGET_VERSION" \
  --skip-key \
  --skip-cli-install; then
  UPDATE_FAILED="1"
fi

if [ "$UPDATE_FAILED" = "0" ] && ! ALGOMIM_HOME="$ALGOMIM_HOME" sh "$STAGED_CLAUDE_CODE/doctor.sh" \
  --credential-profile "$CREDENTIAL_PROFILE" \
  --skip-api-check; then
  UPDATE_FAILED="1"
fi
...
if [ "$UPDATE_FAILED" = "1" ]; then
  printf '[warn] Update failed; restoring Claude Code integration %s.\n' "$INSTALLED_VERSION" >&2
  restore_installation "$INTEGRATION_HOME" "$BACKUP_ROOT"
  echo "Claude Code update rolled back." >&2
  exit 1
fi

log "Updated Claude Code integration from $INSTALLED_VERSION to $TARGET_VERSION."
rm -rf "$TEMP_ROOT"
