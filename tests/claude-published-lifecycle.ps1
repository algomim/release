...
try {
  ...
  Assert-Equal "claude-algomim" ([string] $settings.model) "published install selects the Claude transport model"
  Assert-Equal "1" ([string] @($settings.availableModels).Count) "published install exposes one named model"
  Assert-Equal "claude-algomim" ([string] @($settings.availableModels)[0]) "published install allows only the Claude transport model"
  Assert-True ($settings.enforceAvailableModels -eq $true) "published install enforces the allowlist for Default"
  ...
  Assert-Equal "claude-algomim" ([string] $settings.env.ANTHROPIC_MODEL) "published install selects the Claude transport model for the main session"
  Assert-Equal "claude-algomim" ([string] $settings.env.ANTHROPIC_CUSTOM_MODEL_OPTION) "published install adds the Algomim custom model option"
  ...
  Assert-Equal "claude-algomim" ([string] $settings.env.CLAUDE_CODE_SUBAGENT_MODEL) "published install redirects subagents"
