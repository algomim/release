...
assert_equal "0.3.1" "$(json_field version "$CLI_STATE")" "CLI state must record the release version"
assert_equal "v0.3.1" "$(json_field releaseTag "$CLI_STATE")" "CLI state must record the immutable tag"
...
sh "$CLI" version | grep -F 'Algomim CLI 0.3.1 (v0.3.1)' >/dev/null || fail "version must report CLI version"
