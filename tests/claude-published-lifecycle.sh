...
grep -q '"model"[[:space:]]*:[[:space:]]*"claude-algomim"' "$SETTINGS_PATH" || fail "published install must select the Claude transport model"
grep -q '"availableModels"[[:space:]]*:[[:space:]]*\[[[:space:]]*"claude-algomim"[[:space:]]*\]' "$SETTINGS_PATH" || fail "published install must allow only the Claude transport model"
grep -q '"enforceAvailableModels"[[:space:]]*:[[:space:]]*true' "$SETTINGS_PATH" || fail "published install must enforce the allowlist for Default"
...
assert_equal "claude-algomim" "$(json_field ANTHROPIC_MODEL "$SETTINGS_PATH")" "published install must select the Claude transport model for the main session"
assert_equal "claude-algomim" "$(json_field ANTHROPIC_CUSTOM_MODEL_OPTION "$SETTINGS_PATH")" "published install must add the Algomim custom model option"
...
assert_equal "claude-algomim" "$(json_field CLAUDE_CODE_SUBAGENT_MODEL "$SETTINGS_PATH")" "published install must redirect subagents"
