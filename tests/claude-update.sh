...
BASELINE_VERSION="0.3.7"
BASELINE_TAG="v$BASELINE_VERSION"
BASELINE_REVISION="7f92045602849931c1eb7b2c1a737c886c3971c7"
CANDIDATE_VERSION="0.3.8"
...
BASELINE_ARCHIVE="$TEST_ROOT/claude-v0.3.7.tar"
...
assert_equal "$BASELINE_VERSION" "$(json_field version "$STAGE/baseline/claude-code/release.json")" "baseline contract must record v0.3.7"
...
assert_equal "$BASELINE_VERSION" "$(json_field version "$ALGOMIM_HOME/cli/state.json")" "test must install the immutable v0.3.7 CLI"
...
assert_equal "$BASELINE_VERSION" "$(json_field version "$STATE_PATH")" "test must start from immutable v0.3.7"
...
assert_equal "$CANDIDATE_VERSION" "$(json_field version "$STATE_PATH")" "v0.3.7 CLI and updater must install candidate v0.3.8"
...
printf '[ok] POSIX Claude Code v0.3.7 to candidate v0.3.8 update and rollback tests passed.\n'
rm -rf "$TEST_ROOT"
