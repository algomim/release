...
The profile sets `web_search = "live"`, and the generated catalog advertises
native search support. Codex can therefore send its built-in `web_search` tool
to Algomim Inference; search execution remains hosted and does not require a
local MCP server.
...
Existing `v0.1.2` users run the `v0.3.10` one-liner once. Their credential is
...
```powershell
irm https://raw.githubusercontent.com/algomim/release/v0.3.10/codex/install.ps1 -OutFile install.ps1
.\install.ps1 -BaseUrl "https://example.ngrok-free.dev/v1"
Remove-Item .\install.ps1
```

```sh
curl -fsSL https://raw.githubusercontent.com/algomim/release/v0.3.10/codex/install.sh -o install.sh
...
- The profile selects Algomim, uses the Responses wire API, and enables native
  web search.
...
For Algomim, Codex `/effort` exposes only `minimal`, `low`, `medium`, and
`high`, with `medium` as the default. The upstream model requires reasoning, so
`none` is intentionally unavailable; unsupported `xhigh` and `max` values are
also omitted from the generated catalog.
